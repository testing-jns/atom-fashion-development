...
$email = $view_data["results"]["email"];
$sql = "SELECT whislist_id FROM customers WHERE email = '$email'";
$response = $db->query($sql)->execute();
$wishlist_id = $response->result()->whislist_id;


$sql = "SELECT product_code FROM whislists WHERE id = '$wishlist_id'";
$response = $db->query($sql)->execute();
$product_code_from_wishlists = $response->resultAll();


$wishlist_products = array_map(function ($product) use($db) {
    $sql = "SELECT code, `name`, category, price, discount, rating, thumbnail_id FROM products WHERE code = '$product->product_code'";
    $response = $db->query($sql)->execute();
    return $response->result();
}, $product_code_from_wishlists);
...
<div class="container">
  ...
  <!--
    - custom js link
  -->
  <script src="<?= BASE_URL ?>assets/js/user/wish-cart/script.js"></script>
