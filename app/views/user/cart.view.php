...
$sql = "SELECT cart_id FROM customers WHERE email = '$email'";
$response = $db->query($sql)->execute();
$cart_id = $response->result()->cart_id;


$sql = "SELECT product_code FROM carts WHERE id = '$cart_id'";
$response = $db->query($sql)->execute();
$product_code_from_carts = $response->resultAll();


$cart_products = array_map(function ($product) use($db) {
    $sql = "SELECT code, `name`, category, price, discount, rating, thumbnail_id FROM products WHERE code = '$product->product_code'";
    $response = $db->query($sql)->execute();
    return $response->result();
}, $product_code_from_carts);
